...
(test (parse '"Hola") (strinGS "Hola"))
(test (parse '{sub1 2}) (opS sub1 (list (numS 2))))
...
(test (parse '{rec {{foo {fun {x} x}} {a 10} {b {+ 1 a}}} {foo b}})
      (rec
          (list (binding 'foo (funS '(x) (idS 'x))) (binding 'a (numS 10)) (binding 'b (opS + (list (numS 1) (idS 'a)))))
        (appS (idS 'foo) (list (idS 'b)))))
